...
The linear readout layer is thus trained to minimize the squared output error $E
= \Vert \mathbf{y}(t) - \mathbf{\hat{y}}(t) \Vert$ where $\mathbf{\hat{y}}(t)$
is the target output, which amounts to solving a system of linear equations
...
\noindent where $\lambda_{r}$ is a regularization coefficient, and Moore-Penrose
pseudo-inversion
