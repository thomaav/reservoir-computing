...
\cite{montavon_practical_2012, jaeger_tutorial_2002}, suggesting parameters
...
\textcolor{red}{
  Echo State Property and short-term memory. In practice amounts mostly to input
scaling and spectral radius. This is the memory nonlinearity trade-off.
}

\subsection{Improvements to the Traditional ESN}

\textcolor{red}{
  All beyond the scope of the thesis -- not what we are investigating.
}

\textcolor{red}{
  Some minor details about work that has been done to improve upon the model,
such as intrinsic plasticity, noise, leaking neurons, lateral inhibitions, and
so on. Also deep ESNs now (most recent, actually quite recent). Ensembles
...
\textcolor{red}{
  Maybe something about the equation if we end up using it.
}
...
\subsection{Physical Reservoirs Requirements}
